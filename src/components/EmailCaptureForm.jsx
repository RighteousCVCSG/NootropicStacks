  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/email/subscribe', {
        ...
        body: JSON.stringify({ email, source, leadMagnet, articleSlug }),
      });
      ...
      // Network failure (offline, backend down) — fall back to local
      // capture so the user still gets a success state, and the lead isn't
      // lost outright. Stored under "ns_pending_subscribers" for later export.
      try {
