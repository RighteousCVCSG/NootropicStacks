...
const INLINE_TEXT = (
  <>
    ...
    <a href="/affiliate-disclosure" className="underline underline-offset-2 hover:text-ink-900 transition-colors">
...
const STACKS_TEXT = (
  <>
    ...
    <code className="text-xs bg-ink-200 px-1 rounded">{AMAZON_TAG}</code>.
...
const VARIANT_CONFIG = {
  banner: {
    ...
    className:
      'mb-2 rounded border border-ink-200 bg-surface-card px-3 py-1.5 text-center text-xs leading-snug text-ink-700',
  ...
  inline: {
    ...
    className:
      'flex items-start gap-3 p-4 mb-6 bg-surface-card border border-ink-200 rounded-lg text-sm leading-normal text-ink-700',
  ...
  stacks: {
    ...
    className:
      'p-4 bg-surface-card rounded-lg border border-ink-200 text-sm text-ink-700 mb-8',
  ...
  compare: {
    ...
    className:
      'text-xs text-ink-400 text-center',
  ...
  if (variant === 'inline') {
    return (
      <div role={config.role} aria-label="Affiliate disclosure" data-testid={config.testId} className={config.className}>
        <svg className="w-5 h-5 mt-0.5 shrink-0 text-ink-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
